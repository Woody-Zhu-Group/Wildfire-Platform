...
test('filter availability follows actual dataset capabilities including a mixed time series', () => {
  ...
  assert.deepEqual(filterSupport(['cpuc', 'epss', 'calfire']), {county: true, utility: 'single', only: 'PG&E'});
...
test('card notes follow visible time-series datasets and the cited source of agent stat cards', () => {
  ...
  assert.match(datasetCaveats(panelDatasets('time_series', {...settings, seriesMode: 'regional'}))[0], /rows only for PG&E/);
  assert.match(datasetCaveats(panelDatasets('map', {...settings, dataset: 'epss'}))[0], /rows only for PG&E/);
