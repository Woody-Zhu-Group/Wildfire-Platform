...
4. Tools run: the router's deterministic call when it has one; otherwise Jev tool pick, template answers, or the slot planner for multi-part questions. The tool executor refuses any read for a utility or period outside its dataset's measured coverage (the loaders write `shared/dataset_coverage.json`; coverage is never declared by hand) with a `not_covered` result, never a zero, and the answer becomes a clarification on every path. It offers other data only where the coverage file's per-year counts show rows for that utility in that period, and says when an offer drops a filter.
