...
1. Router hard backstops fire first (`services/agent/routing.py`): live and current, future dates, city_needs_place, hftd_constraint_unavailable. Unsupported-topic keywords (cost, leadership, optimization, damage, and the rest of `routing.TOPIC_JUDGMENT_RULES`) are refusals in off mode; in decide mode Jev's off_topic decides them at the decline gate and the keyword rule is the fallback (issue #97, `docs/JEV_DECIDE.md`). The advice rule stays with the router.
