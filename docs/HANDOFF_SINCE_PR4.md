...
| I | An EPSS question naming a utility other than PG&E is clarify / `epss_non_pge_utility`, because EPSS covers PG&E only and a count would be absent, not zero (Michael, 2026-09-24; PR #82). Extended to comparison routes on 2026-09-24 (branch `epss-comparison-guard`): an EPSS comparison with no PG&E named clarifies; the `cases.json` label for `holdout_compare_epss_sce_sdge_2023` still says answer and is unchanged pending a written relabel | `jev_holdout.json`, `jev_holdout_v2.json` | ho_080, hv2_002 |
