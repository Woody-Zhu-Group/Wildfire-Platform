...
| `ignition_definition_<utility>` (and `_period_a`, `_period_b`) | Every utility-scoped CPUC ignition count, paired with the same-period spatial containment count by a companion call, in either direction (`_is_attribute_utility_ignition`, `_is_spatial_comparison_ignition`, `_is_spatial_utility_ignition`). |
...
5. **EPSS cause codes**: the source mixes codes and words (`VEG` 1 row and `Vegetation` 1,042; `UNK` 6 and `Unknown` 3,724; `3RD`, `EF`). Decide with Michael whether a cause filter should fold a code into its word; today each stays its own value.
