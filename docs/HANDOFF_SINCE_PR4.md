...
| `ambiguous_risk_metric` | riskiest, most risky, highest risk; or a ranking or comparison whose measure resolves to none of the registry's measures (`_unresolved_measure`: any word of the measure phrase that is neither `MEASURE_TERMS` nor `MEASURE_QUALIFIER_WORDS`, where the phrase is the words after most, more, or rank ... by, or what a comparison compares in any word order: after compare, the subject before it, or after two joined entities. A superlative or comparative word such as scariest or worse counts as such a word. A not-in-data word such as damage or rate is left to the refusals) | Which utility is riskiest? / Which utility had the most dangerous fires in 2023? | Which risk measure and period? / Names the user's word, keeps the grouping and period, lists `RANK_MEASURES` and `COMPARE_MEASURES` for that grouping with their `MEASURE_LABELS`, says `MEASURES_NOT_IN_DATA` are not in the data, and asks which measure |
...
- **Route snapshot** (`tests/agent/fixtures/route_snapshot.json`, `tests/agent/route_snapshot.py`, PR #58): the path and rule for all 245 dev and holdout v1 questions. The test fails whenever a router change moves a route without the fixture being regenerated by `python -m tests.agent.route_snapshot`, which is the route report `CLAUDE.md` requires. It has been regenerated deliberately since for PR #28 (`ho_073`), PR #68 (three US-sample rows), and PR #94 (six unresolved-measure rows that now ask which measure: `ho_062` and `hv2_059`, "safest", from model; `hv3_039` from `ranking_missing_slots`; `hv3_043` and `hv3_071` from model; and `hv3_044`, "most equipment failures", from `unsupported_ranking`).
