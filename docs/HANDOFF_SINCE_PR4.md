...
| `ambiguous_risk_metric` | riskiest, most risky, highest risk; or a ranking or comparison whose measure resolves to none of the registry's measures (`_unresolved_measure`: the words after most, more, rank ... by, or compare the ... of, or a superlative or comparative word such as scariest or worse, are neither `MEASURE_TERMS` nor `MEASURE_QUALIFIER_WORDS`) | Which utility is riskiest? / Which utility had the most dangerous fires in 2023? | Which risk measure and period? / Names the user's word, keeps the grouping and period, lists `RANK_MEASURES` and `COMPARE_MEASURES` for that grouping with their `MEASURE_LABELS`, says `MEASURES_NOT_IN_DATA` are not in the data, and asks which measure |
...
`tests/agent/test_unsupported_pattern_variants.py` lists the plural and common variant forms for every `UNSUPPORTED` key and fails if a pattern misses one, or if a key is added without variants. It found 44 misses when added (PR #94), including "fatalities", which the old `fatalit\b` never matched. `tests/agent/test_no_em_dash_in_answers.py` fails if any string literal in the agent or shared registry modules, any routed answer for an eval question, or any decline text contains an em dash.
...
| `unsupported_cost` | cost, price, budget, dollar, economic, premium, and their plurals | How much did home insurance premiums rise after the 2024 fires? | No cost data |
...
| `unsupported_leadership` | CEO, chief executive, and their plurals | Who is the CEO of PG&E? | Out of scope |
| `unsupported_optimization` | optimize, optimization, optimal, schedule, scheduling, allocate, allocation (and their inflections), or advice about what a utility or the CPUC should do (`_asks_for_advice`, PR #58) | Should PG&E expand its EPSS program? | Read-only data system, no recommendations |
...
- **Route snapshot** (`tests/agent/fixtures/route_snapshot.json`, `tests/agent/route_snapshot.py`, PR #58): the path and rule for all 245 dev and holdout v1 questions. The test fails whenever a router change moves a route without the fixture being regenerated by `python -m tests.agent.route_snapshot`, which is the route report `CLAUDE.md` requires. It has been regenerated deliberately since for PR #28 (`ho_073`), PR #68 (three US-sample rows), and PR #94 (three v3 unresolved-measure rows, `hv3_039`, `hv3_043`, `hv3_071`, and `ho_064`, whose "property damage costs" now matches the plural cost pattern first).
