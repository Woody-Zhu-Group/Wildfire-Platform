...
| `ambiguous_risk_metric` | riskiest, most risky, highest risk; or a ranking or comparison by a judgment word (dangerous, worst, worse, severe, severity, destructive, damaging, deadly, deadliest, catastrophic, devastating, harmful, serious, bad, hardest hit; `_JUDGMENT_WORD`, `_judgment_word`) | Which utility is riskiest? / Which utility had the most dangerous fires in 2023? | Which risk measure and period? / Names the word, keeps the grouping and period, lists the measures for that grouping (utilities: CPUC ignition counts; acres only by county from CAL FIRE; damage, fatalities, and structures not in the data), and asks which measure |
...
- **Route snapshot** (`tests/agent/fixtures/route_snapshot.json`, `tests/agent/route_snapshot.py`, PR #58): the path and rule for all 245 dev and holdout v1 questions. The test fails whenever a router change moves a route without the fixture being regenerated by `python -m tests.agent.route_snapshot`, which is the route report `CLAUDE.md` requires. It has been regenerated deliberately since for PR #28 (`ho_073`), PR #68 (three US-sample rows), and branch `judgment-words` (three v3 judgment-word rows: `hv3_039`, `hv3_043`, `hv3_071`).
