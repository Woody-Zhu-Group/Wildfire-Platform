...
- `BACKSTOP_RULES` in `services/agent/decisions/decide_mode.py`: the five rules decide mode never sends to Jev. Since issue #97 the `unsupported_<key>` topic keywords (`routing.TOPIC_JUDGMENT_RULES`) are sent to Jev, whose `off_topic` decides them at the decline gate, with the keyword rule as fallback.
...
1. `BACKSTOP_RULES` (live, risk future date, future prediction, city needs place, HFTD constraint) decide without calling Jev. Topic keyword refusals (every other `unsupported_<key>`, and web-search wording) are decided by Jev's `off_topic` at the decline gate since issue #97; below it the keyword rule stands, and the advice rule stays with the router (`docs/JEV_DECIDE.md`).
