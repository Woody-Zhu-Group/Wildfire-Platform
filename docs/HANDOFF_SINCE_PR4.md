...
| `calfire_untagged_utility` | A utility-scoped CAL FIRE result (a utility filter, including `untagged` and a territory spatial summary, a utility comparison, or a period comparison scoped to a utility) with untagged incidents in its scope. On a plain utility filter (one named utility, untagged not included) it states how many CAL FIRE incidents in the same period and scope have no tag and count toward no utility (`untagged_incidents_excluded`); on the other scopes, how many counted incidents have no tag (`untagged_incidents_counted`). Shown only above zero; a result without the needed figure suppresses the answer. |
