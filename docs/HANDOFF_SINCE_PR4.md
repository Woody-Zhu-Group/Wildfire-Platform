...
5. **Tools** (`services/agent/tools.py`, `ToolExecutor.execute`; models in `services/agent/schemas.py`). Seven model-facing tools plus the router-only `risk_surface` and `risk_metrics` (`GET /metrics`, PR #85). The deterministic path passes `harness_call=True`, which exempts the router's own calls from the window hold and lets them send harness-only arguments. County arguments are normalized to the canonical Census name before any call (`harness_county_correction`, PR #76). The model path is `AgentOrchestrator._model_loop` with the OpenRouter-only provider in `services/agent/provider.py` (PR #73). Section 8 lists the guards.
...
7. **Answer text.** Deterministic routes render from tool summaries (`orchestrator._render_deterministic`). The model path synthesizes with a strict schema whose claims cite `evidence_ids`; uncited numbers are rejected and, when the numbers are fine but citations are missing, repaired (`citation_repaired`). If synthesis fails, the tool summary is rendered (`synthesis_fallback_to_tool_summary`). When Jev's intent fact reads compare or trend at or above the decline gate (decide mode), `derived.derive_arithmetic` adds a `harness_arithmetic` evidence item before synthesis with every value computed from the cited counts and their `source_evidence_ids`; the model cites it and never computes (`derived_evidence` trajectory event). Otherwise the figures are withheld from the evidence and the fallback text (`derived_evidence_withheld`).
...
- `BACKSTOP_RULES` in `services/agent/decisions/decide_mode.py`: the five rules decide mode never sends to Jev. Since issue #97 the `unsupported_<key>` topic keywords (`routing.TOPIC_JUDGMENT_RULES`) are sent to Jev, whose `off_topic` refuses at the decline gate and lifts the refusal only at the answer gate, with the keyword rule as fallback.
...
The text of every clarification is completed by `services/agent/clarify_missing.py` (`complete_clarification`, PR #51): when the slots show more than one thing missing, one message asks for all of them and ends with an example rephrasing built only from what the question named. The rule id never changes. Since PR #94 the missing items are read from what the question reads and the router's slots only, never from the rule (year and dataset for event data, grouping for rankings, place and calendar day for risk; a place for counts, maps, and charts is asked only by the router's place rules), so decide mode showing Jev's text instead of the router's cannot drop one; the options offered come from the registry (`RANK_MEASURES` for rankings, `COMPARE_MEASURES` for comparisons, `SERIES_DATASETS` for yearly and seasonal charts), and the `ranking_missing_slots` and `series_mode_missing_dataset` texts are built from them.
...
Decide mode can also return a clarification with a Jev reason id; those ids come from `jev_policy.derive_outcome` and reuse the ids above. Since PR #72, when the router also clarified, the router's rule and text stand and Jev's rule goes to the log only; Jev's own text appears when Jev changes the disposition (a clarify over a router answer or refusal), and, since PR #94, when both clarify and the router's rule is generic (`decide_mode.GENERIC_ROUTER_RULES`, today `ranking_missing_slots`); it then goes through the same `complete_clarification` composition with the router's slots. Also since PR #94, a Jev `rank` or `compare` intent whose measure is `other_measure` clarifies with `ambiguous_risk_metric`, and its text (`services/agent/measure_clarify.py`) keeps the grouping and period and lists the registry's measures for that grouping (`RANK_MEASURES`, `COMPARE_MEASURES`, `MEASURE_LABELS`) and what is not in the data (`MEASURES_NOT_IN_DATA`). Whether the wording names a real measure is Jev's judgment; the router has no measure word list.

### 4.3 Refusals

`tests/agent/test_unsupported_pattern_variants.py` puts every `UNSUPPORTED` key in one of two groups and fails if a key is in neither. Keys whose words are the topic asked for (`cpz`, `air_quality`, `evacuation`, `translation`, `personnel`, `satellite`, `damage`) list their plural and variant forms, and the test fails if a pattern misses one; it found "fatalities", which the old `fatalit\b` never matched (PR #94). Keys whose words are common in passing (`cost`, `leadership`, `optimization`, `live_web`) keep their singular patterns, because plural forms refused questions that only mentioned costs, budgets, schedules, or CEOs; the test checks that such passing mentions are answered. Some singular passing mentions ("I'm writing a cost memo", "our repair schedule depends on it") are still refused, as before PR #94. `tests/agent/test_no_em_dash_in_answers.py` fails if any string literal in the agent or shared registry modules, any routed answer for an eval question, or any decline text contains an em dash.

| Rule id | Triggers | Example | Why |
|---|---|---|---|
| `unsupported_cpz` | CPZ, circuit protection zone, and their plurals | Which CPZ had the most ignitions in 2024? | No CPZ polygons in the warehouse |
...
| `unsupported_damage` | property damage, expected or insured loss, fatality, and their plurals | How many fatalities did PG&E fires cause in 2018? | Not in the warehouse |
...
Decide mode can also refuse with three Jev-only ids: `prompt_injection` (Jev's injection fact above 0.5), `other_off_topic` (Jev's off-topic choice), and `unsupported_other_measure` (a measure the warehouse cannot return, for a count, trend, or list intent; on a rank or compare intent it asks which measure instead, PR #94). Their text comes from `_REASON_TEXT` and `_GENERIC_UNSUPPORTED` in `decide_mode.py`.
...
- **Date handling** (`services/agent/time_resolve.py`, `resolve_time`): explicit years, calendar days, month plus year, month ranges inside one year (PR #66, `explicit_month_range_in_year`), year ranges, relative years (last year, N years ago), open ranges ending today ("from January 2024 up to today", "since 2020", PR #48, `open_ended_range`), apostrophe years with the `%y` pivot (`'24` is 2024, `'99` is 1999, then out of coverage), bare 1900s years out of coverage (PR #50), vague words ambiguous, and per-year questions marked `per_year` so the harness keeps one call per year. A written year range is always one span, whatever the question asks about it (no change-word list; see the year and range guards row for how the model's own per-period calls are kept). Two named months ("July 2024 and August 2024") resolve to the window from the first month to the last with `per_year` (`named_months`, which counts a month word only where `months_from_text` reads a date or in a list of months ending in a year, so "which may be higher" never adds May), and the router defers a count over them (`_enumerated_months`) instead of counting one month. On the model path each named month is a coverage entity (`named_month_periods`, `month:YYYY-MM`), so a July read alone never answers a July and August question. The first year is measured, not declared: `DATA_YEAR_MIN` is the first year any dataset has rows in `shared/dataset_coverage.json` (2009), and a later year is checked against the asked dataset's own coverage (`dataset_not_covered`), so CPUC 2013 names CPUC's dates. Coverage is measured per query definition (PR #93 final check): CAL FIRE's 2013 rows all have no incident type, so under the old Wildfire/Fire default a CAL FIRE count for 2013 was `dataset_not_covered` while `incident_type_mode` all or untyped answered it; since the default counts untyped incidents (branch `calfire-default`, `docs/DATA_CHANGE_CALFIRE_DEFAULT.md`) the default answers 2013 and 2016 too, and only 2010 to 2012 have no default rows. A month range that crosses a year ("November to February 2023") is ambiguous and asks for the years. A month word counts as a month only where a date can stand (`months_from_text`, PR #93): next to a year or a day ("August 2023", "June 15", "15 June"), after in, during, or for ("in March", "during the month of May", "for early June"), or ending the question or a sentence ("... cpuc august"). Elsewhere "may" and "march" are ordinary words ("which may be higher", "so I can march this to my boss"), a comma included ("you may, if needed"). The slot planner reads the same function.
...
| Year and range guards | `time_resolve.apply_harness_years` and `_hold_resolved_window` (PRs #21, #48, #50) | Out-of-coverage years always fail; a wrong model year is overridden with the harness year; an invented year with no harness year is rejected (`year_not_derived`); with `hold_window` a model call may not narrow a resolved span ("2021 to 2025" with `year=2023` becomes the full span); per-year questions keep per-year calls; listed years reject unlisted ones. Widening applies only when a single call narrows the range with no other call covering the rest: when the model's calls for one question, in one turn or across turns, name distinct periods, years or months alike (`CallWindows.distinct_periods` over every call's `call_window`, collected across all model turns of the question), every call is kept as written, whatever the question's wording; with Jev's compare or trend reading, a lone call on one endpoint year is kept too (`CallWindows.endpoints`), and coverage asks for the other. Each correction is a `harness_time_correction` event. Two model calls that correct to the same backend call run once (`duplicate_tool_call_suppressed`, keyed on the executed arguments), and the deterministic fallback never renders the same line twice. For coverage, a written range names only its endpoints when decide mode has Jev's facts and Jev's intent fact reads compare or trend at or above the decline gate (`AgentOrchestrator._jev_reads_change`, from `jev_intent` and `jev_intent_confidence` on the `jev_decide` slot, carried in `DecideResult.extra`); otherwise (count or records intent, below the gate, Jev error, Jev off) every year in the range must be covered, as before, so a total read from two endpoints still declines. Derived arithmetic (`derived.py`) computes difference, percent change, and ratio from call structure alone: an entity read in two or more periods gets its change over time, and two entities get their difference only when every call shares one period; the figures attach only under the same Jev compare or trend reading. Named months are coverage entities like years (`named_month_periods`). |
...
| `calfire_missingness` | A CAL FIRE result (or a spatial summary's CAL FIRE count) that counted incidents with no type recorded; states that number from the result's `untyped_incidents_counted`. A CAL FIRE result without that figure suppresses the answer. Before branch `calfire-default` it attached to every CAL FIRE read and stated the table-wide untyped and untagged counts (`docs/DATA_CHANGE_CALFIRE_DEFAULT.md`). |
| `calfire_untagged_utility` | A utility-scoped CAL FIRE result (a utility filter, including `untagged` and a territory spatial summary, a utility comparison, or a period comparison scoped to a utility) with untagged incidents in its scope. On a count by utility tag (a plain utility filter, or a utility or single-utility period comparison with the attribute definition) it states how many CAL FIRE incidents in the same period and scope have no tag and count toward no utility (`untagged_incidents_excluded`); on the other scopes (`include_untagged`, `untagged`, territory summaries, spatial comparisons), how many counted incidents have no tag (`untagged_incidents_counted`). The `calfire_missingness` text follows the `incident_type_mode` the count used, so all-types and untyped-only answers never say non-wildfire types were excluded. Shown only above zero; a result without the needed figure suppresses the answer. |
...
1. `BACKSTOP_RULES` (live, risk future date, future prediction, city needs place, HFTD constraint) decide without calling Jev. Topic keyword refusals (every other `unsupported_<key>`, and web-search wording) are decided by Jev's `off_topic` since issue #97 (an off-topic reading refuses at the decline gate, `on_topic` lifts the refusal only at the answer gate); below those gates the keyword rule stands, and the advice rule stays with the router (`docs/JEV_DECIDE.md`).
2. `REGEX_ONLY` rules and routes using a router-only tool (`risk_surface`) stay with the router.
3. Otherwise Jev's outcome is compared with the router's. A Jev clarify or refuse wins at or above the decline gate `AGENT_JEV_DECIDE_MIN_CONFIDENCE` (0.8), unless it asks for a time or place the router already resolved (`contradicts_slot`) or one that the router's chosen deterministic calls do not take (`slot_unused`, added 2026-09-24 after a production territory lookup drew a year question; the missing-year policy gates now apply only to intents whose tools take a time window, `jev_policy.needs_time_window`). A Jev answer over a router decline wins only at or above the answer gate `AGENT_JEV_DECIDE_ANSWER_CONFIDENCE` (0.9) measured on the facts behind the router's rule (`_RULE_FACTS`), and never when the router's resolver proved the time or place missing in code (`code_verified`; `time_out_of_coverage` has no Jev fact and is never overridden). Since PR #94 a ranking refusal the registry proves (two datasets named, or a dataset and grouping pair not in `RANK_MEASURES`) is `code_verified` too: neither a Jev clarification nor a Jev answer replaces it (`decide_mode.registry_verified_refusal`). Below a gate, on a timeout (`AGENT_JEV_TIMEOUT_SECONDS`, 3 s, one bounded pool of 8 threads), or on any error, the router stands.
4. **Jev owns the disposition, the router owns the wording** (PR #72). When Jev wins a decline and the router declined the same way (both clarify, or both refuse), the final decision keeps the router's text, rule, reason, and clarify-all-missing additions; Jev's rule goes to the log only (`wording: "router"`). When Jev changes the disposition, its clarification goes through `complete_clarification` with the router's slots (`wording: "jev"`). The production case that forced this: "Show PSPS events around Santa Rosa" got Jev's generic "What latitude/longitude or bounding box should I use?" (Jev `missing_location` at 0.88) instead of the router's `undefined_spatial_scope` question plus the missing year. On the 307 stored rows the rule changed 9 texts (7 `missing_location` to `undefined_spatial_scope`, 2 `ranking_missing_year` to `ranking_missing_slots`) and no disposition or winner. PR #94 added one exception: a generic router clarification (`GENERIC_ROUTER_RULES`, today `ranking_missing_slots`) yields to Jev's more specific one, which moved those two rows back to Jev's `ranking_missing_year` text.
...
- A `jev_decide` line on stdout and in `AGENT_JEV_LOG_PATH` for every disagreement, every Jev decline whose rule differs from the final rule, and every error or timeout: the router path and rule, Jev's disposition, rule, and confidence, the `winner` (`router` or `jev`), the `why` (`agree`, `gate`, `below_gate`, `contradicts_slot`, `slot_unused`, `code_verified`, `regex_only`, `router_only_tool`, `backstop`, `error`, `timeout`), and `wording` (`router`, `jev`, or null). `services/agent/eval/shadow_report.py` reads the same file.
...
| I | An EPSS question naming a utility other than PG&E is clarify / `epss_non_pge_utility`, because EPSS covers PG&E only and a count would be absent, not zero (Michael, 2026-09-24; PR #82). Extended by Michael on 2026-09-24 (PR #93): a comparison on an EPSS metric that names no PG&E utility is clarify / `epss_non_pge_utility`; a comparison that names PG&E answers with the non-PG&E side null and its reason. The extension relabeled `holdout_compare_epss_sce_sdge_2023` from deterministic `comparison_run` answer to clarification, with the previous label kept under `label_rule.previous`. Coverage is measured by the loaders (`shared/dataset_coverage.json`, PR #93 re-review) and read on every path, and a count for EPSS inside a non-PG&E result (a spatial summary for SCE territory) is shown as not covered, never 0. The same measured check covers every dataset and period (`dataset_not_covered`, rule K below) | `jev_holdout.json`, `jev_holdout_v2.json`, `cases.json` | ho_080, hv2_002, holdout_compare_epss_sce_sdge_2023 |
...
| K | A question whose dataset has no measured coverage for the named utility and period is clarify / `dataset_not_covered`, because a count would be absent, not zero (Michael, 2026-09-24; PR #93). Coverage is measured by the loaders (`shared/dataset_coverage.json`). Each row keeps its previous label (`label_rule.previous` in `cases.json` and `jev_paraphrases.json`, `previous` in the holdouts and the v3 labels file), and `needs_human_review` is cleared on the rows rule K decided | `cases.json`, `jev_paraphrases.json`, `jev_holdout.json`, `jev_holdout_v2.json`, `jev_holdout_v3_labels_chatgpt.json` (row and rule record) | collision_this_year_count, para_bear_valley, para_liberty_2021, ho_016, ho_024, hv2_007, hv2_012, hv2_030, hv3_016 |
...
**Database.** The production PostGIS warehouse runs on Amazon RDS, not on the EC2 host. The services on the host connect to the RDS endpoint (host name starting `wildfire-db`, under `rds.amazonaws.com`) through the `POSTGRES_*` settings in the host's `.env` (reported by Michael, 2026-09-24). The HFTD reload below runs on the host against that endpoint. `docker-compose.yml` (PostGIS on port 5433) is for local development only.
...
- **Backstop**: a router rule that fires before Jev is consulted (`BACKSTOP_RULES`): live, future, city, and HFTD constraint; advice is router-only too. Since issue #97 the unsupported-topic keywords are decided by Jev in decide mode, with the keyword as fallback.
...
- **Decide mode**: `AGENT_JEV_MODE=decide`, where Jev decides answer, clarify, or refuse behind two gates with the router as backstop; Jev owns the disposition and the router owns the wording, except that a generic router clarification yields to Jev's more specific one (PR #94). On in production since 2026-09-24.
