...
| Measures a ranking or comparison can order by | `dataset_registry.py` | `RANK_MEASURES` (derived from `ALLOWED_RANK_PAIRS`), `COMPARE_MEASURES` (per comparison scope), `MEASURE_DATASETS`, `MEASURE_UTILITIES` | router measure clarification, Jev policy |
| Measure names and wording | `naming.py` | `MEASURE_LABELS`, `MEASURES_NOT_IN_DATA`, `MEASURE_TERMS`, `MEASURE_QUALIFIER_WORDS`, `NOT_IN_DATA_MEASURE_PATTERN`, `NOT_A_MEASURE_SUPERLATIVE` | router `_unresolved_measure`, Jev policy |
