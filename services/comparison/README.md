# Comparison service

Cross-utility / region / period metric aggregates for the wildfire warehouse.

## Run

```powershell
# from the repo root
$env:PYTHONPATH = "."
uvicorn services.comparison.app:app --port 8003 --app-dir .
```

Docs: http://127.0.0.1:8003/docs

## Endpoints

| Path | Purpose |
|------|---------|
| `GET /health` | DB ping + metric/definition notes |
| `GET /compare-utilities` | Metric per utility. `utilities` (comma-separated), `metric`, `start_date`, `end_date` required. Each result keeps `key` as the utility code (`PGE`) and also carries `code` and `label` (`PG&E`) from `group_code_and_label` in the naming registry, as `/rank` and `/grouped-counts` rows do |
| `GET /compare-regions` | Metric per county or HFTD tier. `region_type=county\|hftd`, `regions` (comma-separated), `metric`, `start_date`, `end_date` required |
| `GET /compare-periods` | Same scope, two date ranges + delta. `scope_type=utility\|county\|hftd`, `scope`, `metric`, `period_a_start`, `period_a_end`, `period_b_start`, `period_b_end` required |

All three also take `normalize` (default `none`) and `ignition_definition`.

### Metrics

`ignition_count`, `epss_outage_count`, `epss_to_ignition_ratio`, `calfire_incident_count`, `acres_burned`, `psps_event_count`, `customers_deenergized`

### Normalization

`normalize=none|per_circuit|per_km2`. The response always labels which form was returned (`value`, `raw_value`, `denominator`). Unavailable denominators → `value: null` + `reason` (never a silent zero). The canvas Comparison view must draw a **hatched placeholder** for those bars, not an empty axis plus a floating “no data” (see [`frontend/CANVAS.md`](../../frontend/CANVAS.md)).

### Definitions

- **Ignitions:** `ignition_definition=attribute` (default for utilities) or `spatial` (`ST_Within`; default for HFTD).
- **Coverage:** measured by the loaders (`shared/dataset_coverage.json`). A value for a utility or period a metric's dataset has no rows for is `null` + the measured reason, never 0, and is not queried (EPSS has PG&E rows only, from 2021-11-01; PSPS from 2021-10-11 for PG&E, SCE, SDG&E, and Liberty; CPUC for PacifiCorp, PG&E, SCE, and SDG&E). A covered period that runs past the measured window carries `coverage_note`. `meta.coverage` is the metric dataset's measured coverage.
- **CAL FIRE:** every incident except the non-wildfire types (Earthquake, Flood, Hazmat; `CALFIRE_NON_WILDFIRE_INCIDENT_TYPES`), so untyped incidents are counted. `meta.calfire_excluded_incident_types` lists the excluded types, and `meta.untyped_incidents_counted` and `meta.untagged_incidents_counted` give the incidents counted with no type and with no utility tag, summed over the compared groups and periods. A utility or single-utility period comparison by tag (`ignition_definition=attribute`) also gives `meta.untagged_incidents_excluded`: the untagged incidents in the compared periods, which count toward no utility (summed over periods, not utilities) (see `docs/DATA_CHANGE_CALFIRE_DEFAULT.md`).
- **County:** attribute on EPSS/CAL FIRE (a CAL FIRE incident that lists several counties, such as "Shasta, Tehama", counts, with its full acreage, in each; `meta.multi_county_incidents` gives the distinct multi-county incidents across the compared counties or periods, and `meta.notes` says county totals can exceed the statewide total); CPUC ignitions use load-time Census PIP `county`. PSPS still has no county column → null + reason. `per_circuit` returns null + `REASON_CIRCUITS_SCOPE` for a county, or for a utility the measured circuits inventory has no rows for. County `per_km2` is not wired yet (`REASON_NO_COUNTY_AREA`) even though `wildfire.counties` now exists.
- **No CPZ** in this warehouse.
