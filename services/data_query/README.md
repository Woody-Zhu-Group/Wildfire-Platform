...
**Row `code` and `label` (issue #89).** `/rank` keys utility rows by code (`PGE`) and `/grouped-counts` keys them by display label (`PG&E`). Both keys are unchanged, and every row of both responses also has `code` and `label` from `group_code_and_label` in `services/shared/naming.py`, so the same utility reads the same in both: `{"code": "PGE", "label": "PG&E"}`. A utility the registry does not list, and the missing-value rows (`(unknown)` on `/rank`, `Not recorded` on `/grouped-counts`), use their key as both. Rows grouped by county, circuit, or cause use the key as both: a county name, a 9-digit circuit ID (the circuit's name stays in `circuit_name`), or a cause. Callers should display `label` and filter with `code`.
