...
> Historical record (2026-09-24): notes from the qwen3:4b to qwen3:32b investigation. The local Ollama/qwen path, `model_setup.py`, `AGENT_MODEL`, `AGENT_NUM_CTX`, and the thinking and structured-mode settings described below were all removed on 2026-09-24; the agent runs on OpenRouter (GPT-6 Luna, see [`docs/OPENROUTER.md`](docs/OPENROUTER.md)). Nothing here applies to the current code. Statements below about the `qwen3:4b` runtime default describe the code at the time they were written.
>
