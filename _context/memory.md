...
> Status (2026-09-23): an append-only dated session log (2026-08-07 to 2026-09-22), accurate as history. Read as a description of current code it is out of date: the agent has seven grouped tools, cases.json has 107 cases, the runtime model default is `qwen2.5:7b`, the current website is `website/` (not the older `frontend/`), the risk API also serves `/surface`, `/observed`, `/observed-training`, and `/metrics`, and OpenRouter plus the Jev `tool_pick` and `tool_pick_template` modes exist (all off by default). See the root README for the current state.
